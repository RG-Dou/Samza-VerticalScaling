...
import org.apache.samza.zk.FollowerJobCoordinatorFactory;//DrG
...
public class NewStreamProcessor {
    private static final Logger LOGGER = LoggerFactory.getLogger(NewStreamProcessor.class);
    private static final String CONTAINER_THREAD_NAME_FORMAT = "Samza NewStreamProcessor Container Thread-%d";

    private final FollowerJobCoordinator jobCoordinator;
    ...
    /**
     * Same as {@link #NewStreamProcessor(Config, Map, TaskFactory, ProcessorLifecycleListener, JobCoordinator)}, except
     ...
     * @param processorListener listener to the NewStreamProcessor life cycle
     *
     ...
     */
    @Deprecated
    public NewStreamProcessor(Config config, Map<String, MetricsReporter> customMetricsReporters, TaskFactory taskFactory,
                           ProcessorLifecycleListener processorListener) {
        this(config, customMetricsReporters, taskFactory, processorListener, null);
    }

    /**
     * Same as {@link #NewStreamProcessor(Config, Map, TaskFactory, Optional, Optional,
     ...
     *
     * Deprecated: Use {@link #NewStreamProcessor(Config, Map, TaskFactory, Optional, Optional,
     * StreamProcessorLifecycleListenerFactory, JobCoordinator)} instead.
     */
    @Deprecated
    public NewStreamProcessor(Config config, Map<String, MetricsReporter> customMetricsReporters, TaskFactory taskFactory,
    ...
    /**
     * Builds a {@link NewStreamProcessor} with full specification of processing components.
     *
     * @param config configuration required to launch {@link JobCoordinator} and {@link SamzaContainer}
     * @param customMetricsReporters registered with the metrics system to report metrics
     * @param taskFactory task factory to instantiate the Task
    ...
    public NewStreamProcessor(Config config, Map<String, MetricsReporter> customMetricsReporters, TaskFactory taskFactory,
        ...
        this.jobCoordinator = (jobCoordinator != null) ? (FollowerJobCoordinator) jobCoordinator : createJobCoordinator();
    ...
    /**
     * Asynchronously starts this {@link NewStreamProcessor}.
     * <p>
     *   <b>Implementation</b>:
    ...
    /**
     * <p>
     * Asynchronously stops the {@link NewStreamProcessor}'s running components - {@link SamzaContainer}
     * and {@link JobCoordinator}
     * </p>
     * Here're the ways which can stop the NewStreamProcessor:
     * <ol>
     *   <li>Caller of NewStreamProcessor invokes stop()</li>
     ...
     * NewStreamProcessor. This needs to be synchronized so that only one code path gets triggered for shutdown.
     * <br>
    ...
    public void stop() {
        synchronized (lock) {
            if (state != State.STOPPING && state != State.STOPPED) {
                ...
                LOGGER.info("NewStreamProcessor state is: {}. Ignoring the stop.", state);
            }
        }
    }
    ...
    private FollowerJobCoordinator createJobCoordinator() {
        String jobCoordinatorFactoryClassName = FollowerJobCoordinatorFactory.class.getName();
        return (FollowerJobCoordinator)Util.getObj(jobCoordinatorFactoryClassName, FollowerJobCoordinatorFactory.class).getJobCoordinator(config);
...
//        return Util.getObj(jobCoordinatorFactoryClassName, JobCoordinatorFactory.class).getJobCoordinator(config);
    ...
    private FollowerJobCoordinator.FollowerJobCoordinatorListener createJobCoordinatorListener() {
        return new FollowerJobCoordinator.FollowerJobCoordinatorListener() {
...
//                String result = "1";
    ...
    public interface StreamProcessorLifecycleListenerFactory {
        ProcessorLifecycleListener createInstance(NewStreamProcessor processor);
    }
    ...
    class ContainerListener implements SamzaContainerListener {
        ...
        public void beforeStart() {
            // processorListener.beforeStart() is invoked in NewStreamProcessor.start()
        }
