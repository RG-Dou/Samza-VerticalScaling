...
public class YarnApplicationMaster {
    ...
    private void startLeader(){
        ...
        leaderJobCoordinator.setListener(new JobCoordinatorListener() {
            ...
            public void onNewJobModel(String processorId, JobModel jobModel) {
                ...
                if(jobModel instanceof ResponseModel){
                    log.info("New Resource Model actually deployed in container: " + processorId);
                    ResponseModel responseModel = (ResponseModel) jobModel;
                    String response = responseModel.getResponse();
                    controller.onResourceResized(processorId, response);
                } else {
    ...
    private void startController(Map<String, ContainerModel> containers){
        ...
        controller.init(new OperatorControllerListener() {
            ...
            @Override
            public void containerResize(String processorId, int numCores, int memoryMb) {
                log.info("Start resize container for " + processorId + ", the target number of CPU cores is " + numCores + ". memoryMb is " + memoryMb);
                containerProcessManager.resizeContainer(processorId, numCores, memoryMb);
            }
