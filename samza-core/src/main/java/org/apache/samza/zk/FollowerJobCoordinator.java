...
public class FollowerJobCoordinator implements JobCoordinator {
    ...
    //DrG
    private String getModelData(String modelVersion){
        LOG.info("Read the model ver=" + modelVersion + " from " + zkUtils.getKeyBuilder().getJobModelPath(modelVersion));
        Object data = zkUtils.getZkClient().readData(zkUtils.getKeyBuilder().getJobModelPath(modelVersion));
        return data.toString();
    }

    //DrG
    private Boolean isJobModel(String model){
        if(model.contains("\"memModels\"")){
            return false;
        } else{
            return true;
        }
    ...
    public void publishResponse(String result, ResourceModel resourceModel, String version){
        ...
        try {
            ...
            LOG.error("ResponseModel publish failed for version=" + version, e);
