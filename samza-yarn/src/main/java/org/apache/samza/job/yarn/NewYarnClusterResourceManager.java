...
import org.apache.hadoop.yarn.api.protocolrecords.AllocateRequest; //DrG
import org.apache.hadoop.yarn.api.protocolrecords.AllocateResponse; //DrG
...
import org.apache.hadoop.yarn.security.ContainerTokenIdentifier; // DrG
...
/**
 *
 * An {@link NewYarnClusterResourceManager} implements a ClusterResourceManager using Yarn as the underlying
...
//DrG
//public class NewYarnClusterResourceManager extends ClusterResourceManager implements AMRMClientAsync.CallbackHandler, NMClientAsync.CallbackHandler {

public class NewYarnClusterResourceManager extends ClusterResourceManager implements AMRMClientAsync.AbstractCallbackHandler, NMClientAsync.CallbackHandler {
    ...
    //DrG
    private final ConcurrentHashMap<String, SamzaResource> allocatedContainers = new ConcurrentHashMap<>();
    ...
    private static final Logger log = LoggerFactory.getLogger(NewYarnClusterResourceManager.class);
    private final Config config;

    NewYarnClusterResourceManager(AMRMClientAsync amClientAsync, NMClientAsync nmClientAsync, Callback callback,
    ...
    /**
     * Creates an NewYarnClusterResourceManager from config, a jobModelReader and a callback.
     ...
     */
    public NewYarnClusterResourceManager(Config config, JobModelManager jobModelManager, ClusterResourceManager.Callback callback, SamzaApplicationState samzaAppState ) {
        super(callback);
    ...
    public void requestResources(SamzaResourceRequest resourceRequest) {
        ...
        log.info("Allocated Resource for containers: " + allocatedContainers.toString());
        String processorId = resourceRequest.getContainerID();
        for(Map.Entry<String, SamzaResource> entry : allocatedContainers.entrySet()){
            String containerId = entry.getKey();
            if(containerId.contains(processorId)){
                SamzaResource oldResource = entry.getValue();
                changeContainerSize(containerId, resourceRequest, oldResource);
                return;
            }
        }
        addContainer(resourceRequest);
    }

    private void addContainer(SamzaResourceRequest resourceRequest){
        log.info("Request new resources for container " + resourceRequest.getContainerID());
    ...
    private void changeContainerSize(String containerId, SamzaResourceRequest resourceRequest, SamzaResource oldResource){
        int oldMemMb = oldResource.getMemoryMb();
        int oldCores = oldResource.getNumCores();
        int requestMemMb = resourceRequest.getMemoryMB();
        int requestCores = resourceRequest.getNumCores();
        if(requestMemMb == oldMemMb && requestCores == oldCores){
            log.info("The target resourses are equal to the current allocated");
        } else if(requestMemMb >= oldMemMb && requestCores >= oldCores){
            changeContainersRM(containerId, requestMemMb, requestCores, true);
        } else if(requestMemMb <= oldMemMb && requestCores <= oldCores){
            changeContainersRM(containerId, requestMemMb, requestCores, false);
        } else {
            log.info("Wrong target resources to change");
        }
    }

    private void changeContainersRM(String containerIdStr, int mem, int cpu, boolean increased){
        ContainerId containerId = ConverterUtils.toContainerId(containerIdStr);
        List requests = Arrays.asList(
                ContainerResourceChangeRequest.newInstance(containerId, Resources.createResource(mem, cpu)));
        try {
            if(increased) {
                AllocateResponse response = amClient.increaseContainersSize(requests, null);
...
//              }
                ...
                AllocateResponse response = amClient.increaseContainersSize(null, requests);
            }
        } catch (IOException e){
    ...
    private void increaseContainersNM(Container container){
        log.info("Received increased container from NM. Container ID: " + container.getId().toString() + ". Target resource is: " + container.getResource().toString());
    ...
    public void releaseResources(SamzaResource resource) {
        ...
        synchronized (lock) {
            ...
            //DrG
            allocatedContainers.remove(ConverterUtils.toString(container.getId()));
        }
    }
    ...
    public void onContainersAllocated(List<Container> containers) {
        ...
        for(Container container : containers) {
            ...
            //DrG
            allocatedContainers.put(id, resource);
    ...
    public void onContainersResourceChanged(List<Container> containers) {
        ...
        for(Container container: containers){
            increaseContainersNM(container);
        }
    }
...
//        log.info("Container {} has updated successfully. New resource {}", containerId, resource.toString());
        ...
        synchronized (lock) {
            SamzaResource oldResource = allocatedContainers.remove(containerIdStr);
            if (oldResource != null) {
                SamzaResource newSamzaResource = new SamzaResource(resource.getVirtualCores(), resource.getMemory(), oldResource.getHost(), oldResource.getResourceID());
                allocatedContainers.put(containerIdStr, newSamzaResource);
            } else {
                log.info("No containerId {} in allocatedContainers.", containerId);
            }
        }
    }
